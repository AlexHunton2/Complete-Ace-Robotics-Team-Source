...
@Autonomous
public class RedOneGripperAutoMode extends AbstractRobotMode {
...
//        // Move straight to align
        ...
        fsm.addCommand(new NormalizeAnglePID(1));
        fsm.addCommand(new DriveFromOgPosition(42, DriveTrain.RobotDirection.FORWARD, 1));
        fsm.addCommand(new NormalizeAnglePID(1));
        fsm.addCommand(new DriveFromOgPosition(35+8, DriveTrain.RobotDirection.FORWARD, 1));
        fsm.addCommand(new NormalizeAnglePID(1));
        fsm.addCommand(new DrivePID(5+4, 1, DriveTrain.RobotDirection.LEFT));
        fsm.addCommand(new SideGrap(Grabber.SideGripState.UP_UNGRIPPED));
        fsm.addCommand(new DrivePIDRotate(-90, 1));
        fsm.addCommand(new DrivePID(7, .3, DriveTrain.RobotDirection.BACKWARD));
        fsm.addCommand(new FoundationGrabberCMD(true));
        fsm.addCommand(new DoNothing(1));
        fsm.addCommand(new DriveForwardPID(47-18+4, 1));
        fsm.addCommand(
                0.0, new DrivePIDRotate(-90, 1),
                0.0, new TapeMeasure(Intake.tapeActions.OUT)
        );
        fsm.addCommand(new DrivePID(5, 1, DriveTrain.RobotDirection.BACKWARD));
        fsm.addCommand(new FoundationGrabberCMD(false));
        fsm.addCommand(new DrivePID(3, 1, DriveTrain.RobotDirection.RIGHT));
