...
@Autonomous
public class RedAutoMode extends AbstractRobotMode {
    ...
    public void init() {
        ...
        fsm.addCommand(new DriveBackward(distanceToStone-7 +5, 1.0));
        fsm.addCommand(new DriveSidewaysFromOrigin(-2 +3, DriveTrain.RobotDirection.RIGHT, 1.0));
        fsm.addCommand(new EjectAStone());
        fsm.addCommand(new DriveSidewaysFromOrigin(Strategy.STONE_WIDTH*2+7 +3, DriveTrain.RobotDirection.LEFT, 1.0));

        // try to pickup the 2nd
        fsm.addCommand(new DriveForward(14 +5, 1.0));
        fsm.addCommand(new GrabAStone(true));
        fsm.addCommand(new DriveBackward(distanceToStone +5, 1.0));

        fsm.addCommand(new DriveSidewaysFromOrigin(Strategy.STONE_WIDTH*2+5 +3, DriveTrain.RobotDirection.RIGHT, 1.0));
        fsm.addCommand(new EjectAStone());
        fsm.addCommand(new DriveSideways(14 +3, DriveTrain.RobotDirection.LEFT, 1.0));
